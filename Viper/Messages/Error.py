# This code is automatically generated for the Viper viewer project, and the generation code can be found at: https://github.com/HeapHeapHooray/Viper

from Message.Message import Message
import BytesUtils
import Utils
from dataclasses import dataclass

@dataclass
class AgentData:
	AgentID: "LLUUID"
AGENTDATA = AgentData

@dataclass
class Data:
	Code: "S32"
	Token: "Variable 1"
	ID: "LLUUID"
	System: "Variable 1"
	Message: "Variable 2"
	Data: "Variable 2"
DATA = Data


class Error(Message):

	absolute_id = 4294902183 # -- The Full ID of the message

	def __init__(self,bytes_data: bytes):

		self.AgentData = AGENTDATA(*((None,)*1))

		self.Data = DATA(*((None,)*6))

		if bytes_data is None:
			return

		remaining_bytes = Utils.zero_decode(bytes_data)

		unpacked_data,remaining_bytes = BytesUtils.unpack_bytes_little_endian(["uuid",],remaining_bytes)
		self.AgentData = AGENTDATA(*unpacked_data)

		unpacked_data,remaining_bytes = BytesUtils.unpack_bytes_little_endian(["signed int32","variable1","uuid","variable1","variable2","variable2",],remaining_bytes)
		self.Data = DATA(*unpacked_data)


	def convert_to_string(self) -> str:
		return f"""Message Type: Error, Message Absolute ID: 4294902183, Blocks: {self.AgentData},{self.Data}"""

	def convert_to_bytes(self) -> bytes:
		output = b""

		output = output + BytesUtils.pack_bytes_little_endian(["uuid",],self.AgentData.AgentID)

		output = output + BytesUtils.pack_bytes_little_endian(["signed int32","variable1","uuid","variable1","variable2","variable2",],self.Data.Code,self.Data.Token,self.Data.ID,self.Data.System,self.Data.Message,self.Data.Data)

		return output