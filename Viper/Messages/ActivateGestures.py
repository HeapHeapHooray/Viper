...
class ActivateGestures(Message):

	absolute_id = 4294902076 # -- The Full ID of the message
	...
	def convert_to_string(self) -> str:
		return f"""Message Type: ActivateGestures, Message Absolute ID: 4294902076, Blocks: {self.AgentData},{self.Data}"""
	...
	def convert_to_bytes(self) -> bytes:
		...
		for i in range(blocks_count):
			output = output + BytesUtils.pack_bytes_little_endian(["uuid","uuid","unsigned int32",],self.Data[i].ItemID,self.Data[i].AssetID,self.Data[i].GestureFlags)
