...
class ObjectUpdateCompressed(Message):

	absolute_id = 13 # -- The Full ID of the message
	...
	def __init__(self,bytes_data: bytes):
		...
		for i in range(blocks_count):
			unpacked_data,remaining_bytes = BytesUtils.unpack_bytes_little_endian(["unsigned int32","variable2",],remaining_bytes)
			self.ObjectData.append(OBJECTDATA(*unpacked_data))


	def convert_to_string(self) -> str:
		return f"""Message Type: ObjectUpdateCompressed, Message Absolute ID: 13, Blocks: {self.RegionData},{self.ObjectData}"""
	...
	def convert_to_bytes(self) -> bytes:
		...
		for i in range(blocks_count):
			output = output + BytesUtils.pack_bytes_little_endian(["unsigned int32","variable2",],self.ObjectData[i].UpdateFlags,self.ObjectData[i].Data)
