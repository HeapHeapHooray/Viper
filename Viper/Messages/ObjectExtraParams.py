...
{self.ObjectData}"""
	...
	def convert_to_bytes(self) -> bytes:
		...
		for i in range(blocks_count):
			output = output + BytesUtils.pack_bytes_little_endian(["unsigned int32","unsigned int16","unsigned byte","unsigned int32","variable1",],self.ObjectData[i].ObjectLocalID,self.ObjectData[i].ParamType,self.ObjectData[i].ParamInUse,self.ObjectData[i].ParamSize,self.ObjectData[i].ParamData)
