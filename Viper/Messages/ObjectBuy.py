...
{self.ObjectData}"""
	...
	def convert_to_bytes(self) -> bytes:
		...
		output = output + BytesUtils.pack_bytes_little_endian(["uuid","uuid","uuid","uuid",],self.AgentData.AgentID,self.AgentData.SessionID,self.AgentData.GroupID,self.AgentData.CategoryID)

		blocks_count = len(self.ObjectData)
		output = output + BytesUtils.pack_bytes_little_endian(["unsigned byte"],blocks_count)

		for i in range(blocks_count):
			output = output + BytesUtils.pack_bytes_little_endian(["unsigned int32","unsigned byte","signed int32",],self.ObjectData[i].ObjectLocalID,self.ObjectData[i].SaleType,self.ObjectData[i].SalePrice)
