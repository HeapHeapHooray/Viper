...
class PacketHeader:
    ...
    def get_data_size_as_bytes(self):
        size = BytesUtils.calculate_minimum_size(["unsigned byte","unsigned int32","unsigned byte"]) + len(self.extra_bytes)
        return size
    def convert_to_bytes(self) -> bytes:
        return BytesUtils.pack_bytes_big_endian(["unsigned byte","unsigned int32","unsigned byte"],self.flags.convert_to_byte(),self.sequence_number,len(self.extra_bytes)) + self.extra_bytes
